import SynthesizeCleaned
import Canonical
/-! Smoke tests for the cleaned `synthesize` tactic (`SynthesizeCleaned.lean` +
`ProgramByPredicateCleaned.lean`). Elaborating this file runs real searches.
The broken `sort` example from `Synthesize/Examples.lean` is omitted. -/

/-! ## Input–output examples (the former `#synthesize`) -/

-- Addition, from three input–output examples (the problem built by hand in
-- the Rust repo's `lean/Test.lean`).
def f : Nat → Nat → Nat := by
  synthesize
  | f 0 0 = 0
  | f 0 1 = 1
  | f 1 0 = 1
  | f 2 2 = 4
  | f 2 3 = 5
  | f 3 9 = 12
  | f 9 3 = 12
  | f 10 10 = 20

-- The predecessor function, requiring case analysis.
def pred : Nat → Nat := by
  synthesize
  | pred 0 = 0
  | pred 1 = 0
  | pred 2 = 1
  | pred 3 = 2

-- With a premise list: the search may use `Nat.add`, and finds `fun a ↦ a + a`.
def double : Nat → Nat := by
  synthesize 10 [Nat.add]
  | double 0 = 0
  | double 1 = 2
  | double 2 = 4

/-! ## Quantified clauses (the former `#synthesize_pred`) -/

-- First projection, from a single universally quantified clause over two
-- variables. The instantiated equations are `proj 0 0 = 0`, `proj 0 1 = 0`,
-- `proj 1 0 = 1`; the search finds `fun a b ↦ a`.
def proj : Nat → Nat → Nat := by
  synthesize
  | ∀ n m : Nat, proj n m = n

-- A clause without binders is an ordinary input–output example; the
-- quantified clause instantiates to `pred' (n + 1) = n` at the enumerated
-- values of `n`. Ground arithmetic is reduced during translation.
def pred' : Nat → Nat := by
  synthesize
  | pred' 0 = 0
  | ∀ n : Nat, pred' (n + 1) = n

-- With a premise list: the search may use `Nat.add`.
def double' : Nat → Nat := by
  synthesize 10 [Nat.add]
  | ∀ n : Nat, double' n = n + n

-- Both sides of a clause may mention the function. Instantiations of
-- commutativity include the diagonal and both orientations; translation
-- turns them into equational constraints. The quantified clauses are only
-- sampled: the synthesized function satisfies the instantiated equations,
-- not necessarily the predicates themselves.
def add : Nat → Nat → Nat := by
  synthesize 60
  | ∀ n m : Nat, add n m = add m n
  | ∀ n : Nat, add n 0 = n
  | add 1 1 = 2

def mul : Nat → Nat → Nat := by
  synthesize
  | mul 0 0 = 0
  | mul 0 1 = 0
  | mul 2 2 = 4
  | mul 2 1 = 2

/-! ## Existential clauses -/

-- The `∃`-binder is skolemized into a skolem function `y : Nat → Nat`
-- synthesized alongside `zero'`; the instantiated equations are
-- `zero' n (y n) = 0`, and the found witness is reported as an info message.
def zero' : Nat → Nat → Nat := by
  synthesize
  | ∀ x : Nat, ∃ y : Nat, zero' x y = 0

-- The witness genuinely matters: the clause samples to `inv (y n) = n`, so
-- `inv` and `y` are forced jointly (`inv = fun a ↦ a` with `y := fun x ↦ x`).
def inv : Nat → Nat := by
  synthesize
  | ∀ x : Nat, ∃ y : Nat, inv y = x

-- An existential clause mixes with ordinary examples; an `∃`-block with no
-- universal prefix has a constant witness (here `y := 1`).
def one' : Nat → Nat := by
  synthesize
  | ∃ y : Nat, one' y = 1
  | one' 0 = 0
